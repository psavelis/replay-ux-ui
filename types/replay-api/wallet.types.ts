...
export interface TransactionsResult extends Omit<PaginatedResult<Transaction>, 'items'> {
...
export const getEVMAddressValue = (value: string | EVMAddress | undefined): string => {
  if (!value) return '';
  return typeof value === 'string' ? value : value.address;
};

export const getBalanceValue = (balances: Record<string, string | Amount> | undefined, currency: string): number => {
...
export const formatEVMAddress = (address: EVMAddress | string | undefined, short = true): string => {
  const addr = getEVMAddressValue(address);
  if (!addr) return '';
  return short && addr.length > 10 ? `${addr.slice(0, 6)}...${addr.slice(-4)}` : addr;
};

export const getTransactionStatusColor = (status: TransactionStatus): 'success' | 'warning' | 'danger' | 'default' => {
  const colors: Record<TransactionStatus, 'success' | 'warning' | 'danger' | 'default'> = {
    confirmed: 'success',
    pending: 'warning',
    failed: 'danger',
    cancelled: 'danger',
...
export const getCurrencyInfo = (currency: Currency) => {
  const info: Record<Currency, { name: string; symbol: string; decimals: number; isStablecoin: boolean }> = {
  ...
  return info[currency] ?? { name: 'Unknown', symbol: '$', decimals: 2, isStablecoin: false };
};
