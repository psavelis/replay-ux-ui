...
export default function ReviewConfirmForm() {
  ...
  const sections = [
    ...
    },
    {
      ...
      title: 'Game Mode',
      value: GAME_MODE_NAMES[state.gameMode] || state.gameMode || 'Not selected',
      color: 'purple',
    },
    {
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        ...
        >
          <div className="rounded-full bg-gradient-to-br from-purple-500/20 to-cyan-500/20 dark:from-purple-500/30 dark:to-cyan-500/30 p-4 mb-3 border border-purple-500/30 dark:border-cyan-500/30">
            <Icon icon="solar:shield-check-bold-duotone" width={48} className="text-purple-500 dark:text-cyan-400" />
          </div>
        </motion.div>
            ...
            <Card className="hover:shadow-lg transition-all duration-200 bg-default-50 dark:bg-slate-800/50 border border-default-200 dark:border-slate-700 hover:border-purple-400/30 dark:hover:border-cyan-400/30">
              <CardBody className="p-4">
                <div className="flex items-start gap-3">
                  <div className={`rounded-lg p-2.5 bg-${section.color}-100 dark:bg-${section.color}-900/30 border border-${section.color}-200 dark:border-${section.color}-800/50`}>
                    <Icon
                      icon={section.icon}
                      width={22}
        ...
        <Card className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800/50 dark:to-slate-900/50 border border-slate-200 dark:border-slate-700">
          <CardHeader className="flex-col items-start gap-2 pb-2">
            <div className="flex items-center gap-2">
              <Icon icon="solar:cup-star-bold-duotone" width={20} className="text-amber-500" />
              <h4 className="font-semibold text-slate-700 dark:text-slate-300">Match Details</h4>
            </div>
          </CardHeader>
          <Divider className="bg-slate-200 dark:bg-slate-700" />
          <CardBody className="gap-3 pt-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600 dark:text-slate-400">Expected Pool:</span>
              <Chip size="sm" variant="flat" className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-bold">
                ${state.expectedPool?.toFixed(2) || '100.00'}
              </Chip>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600 dark:text-slate-400">Match Type:</span>
              <Chip size="sm" variant="flat" className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
            ...
            <div className="flex items-center justify-between">
              ...
              <Chip size="sm" variant="flat" className="bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-400">
      ...
      >
        <div className="relative">
          <Icon icon="solar:play-circle-bold" width={28} className="text-purple-500 dark:text-cyan-400" />
              ...
              <div className="flex justify-center">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Icon icon="solar:gamepad-bold" width={28} className="text-cyan-400" />
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div className="flex flex-col items-center p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                    <Icon icon="solar:users-group-rounded-bold" width={20} className="text-purple-400 mb-1" />
                    <span className="text-slate-400 text-xs">Queue</span>
                    <span className="font-bold text-white">#{state.matchmaking.queuePosition}</span>
                  </div>
                  <div className="flex flex-col items-center p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                    <Icon icon="solar:clock-circle-bold" width={20} className="text-cyan-400 mb-1" />
                    <span className="text-slate-400 text-xs">Est. Wait</span>
                    <span className="font-bold text-white">{Math.floor(state.matchmaking.estimatedWait / 60)}:{String(state.matchmaking.estimatedWait % 60).padStart(2, '0')}</span>
                  </div>
                  <div className="flex flex-col items-center p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                    <Icon icon="solar:hourglass-bold" width={20} className="text-amber-400 mb-1" />
                    <span className="text-slate-400 text-xs">Elapsed</span>
                    <span className="font-bold text-white">{Math.floor(state.matchmaking.elapsedTime / 60)}:{String(state.matchmaking.elapsedTime % 60).padStart(2, '0')}</span>
                  </div>
                </div>
              </div>
            </CardBody>
